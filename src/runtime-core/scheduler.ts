...
const p = Promise.resolve(); // 创建了一个promise实例
let isFlushPending = false; // 用于判断微任务是否已经创建

// nextTick是做什么的：为了获取视图更新后的数据
export function nextTick(fn?) {
    // 将nextTick接收的函数推到微任务中去，如果用户没有传函数，就返回一个p，就会等待，等待完成后就到微任务中去了
